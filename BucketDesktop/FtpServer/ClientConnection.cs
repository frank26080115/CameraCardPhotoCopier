...
namespace SharpFtpServer
{
    public class ClientConnection : IDisposable
    {
        ...
        private FtpServer _parentServer;
        ...
        public ClientConnection(TcpClient client, FtpServer parentServer)
        {
        ...
        public void HandleClient(object obj)
        {
            ...
            try
            {
                while ((line = _controlReader.ReadLine()) != null)
                {
                    ...
                    Logger.Log(LogLevel.Info, "FTP client " + _clientIP + " : " + cmd + " : " + arguments);
                    ...
                    else
                    {
                        ...
                        if (cmd == "AUTH")
                        {
                            //_cert = new X509Certificate(Resources.server_pvk);
                            _cert = new X509Certificate2(Resources.server_pvk);
...
#region FTP Commands
        ...
        private string FileModificationTime(string pathname)
        {
            ...
            if (pathname != null)
            {
                ...
                {
                    pathname = FtpFilePathMap.WasHere(pathname);
                }
                if (File.Exists(pathname))
                {
        ...
        private string PrintWorkingDirectory()
        {
            ...
            {
                root = FtpFilePathMap.GetRoot();
            }

            string current = _currentDirectory.Replace(root, string.Empty).Replace('\\', '/');
...
#region DataConnection Operations
        ...
        {
            if (_parentServer.new_client_flag == FtpClientStatus.Connected)
            {
                _parentServer.new_client_flag = FtpClientStatus.Transfering;
            }

            long bytes = 0;
            string destpath = FtpFilePathMap.GetNewPath(pathname);

            using (FileStream fs = new FileStream(destpath, FileMode.OpenOrCreate, FileAccess.Write, FileShare.None, 4096, FileOptions.SequentialScan))
            {
                bytes = CopyStream(dataStream, fs);
            }
            Program.FtpFileCnt += 1;
            Program.FtpFileSize += Convert.ToInt64(bytes);

            FtpFilePathMap.WriteWasHere(pathname, destpath);
        ...
        private string AppendOperation(NetworkStream dataStream, string pathname)
        {
            ...
            using (FileStream fs = new FileStream(destpath, FileMode.Append, FileAccess.Write, FileShare.None, 4096, FileOptions.SequentialScan))
            {
                bytes = CopyStream(dataStream, fs);
            }
        ...
        private string ListOperation(NetworkStream dataStream, string pathname)
        {
            ...
            foreach (string file in files)
            {
                ...
                if (file.EndsWith(".washere"))
                {
                    string nf = FtpFilePathMap.WasHere(file);
                    if (File.Exists(nf))
                    {
                        f = new FileInfo(nf);
                    }
